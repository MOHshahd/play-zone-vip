        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-medium">استدعاء الموظف</label>
            </div>
            {callSent ? (
              ...
              <div className={showCallStaff ? "" : ""}>
                {!showCallStaff ? (
                  <button onClick={() => setShowCallStaff(true)}
                    className="w-full py-3 rounded-lg font-bold text-white flex items-center justify-center gap-2"
                    style={{ background: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' }}>
                    <i className="fa-solid fa-bell"></i>
                    <span>استدعاء موظف</span>
                  </button>
                ) : (
                  <div className="bg-gray-800 rounded-xl p-4 space-y-3">
                    <label className="block text-sm text-gray-300">السبب</label>
                    <textarea
                      value={callDescription}
                      onChange={e => setCallDescription(e.target.value)}
                      placeholder="اكتب سبب الاستدعاء (مثال: عطل في الجهاز، عايز مشروب، ...)"
                      className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-yellow-500 resize-none"
                      rows={3}
                      maxLength={500}
                    />
                    <div className="grid grid-cols-2 gap-3">
                      <button onClick={() => { setShowCallStaff(false); setCallDescription(''); }}
                        className="py-3 rounded-lg bg-gray-700 font-bold">إلغاء</button>
                      <button onClick={async () => {
                        if (!callDescription.trim()) return;
                        try {
                          await api.post('/servicerequests', {
                            sessionId: session.backendId || null,
                            deviceId: currentRoom.backendId || '',
                            deviceName: `غرفة ${currentRoom.number}`,
