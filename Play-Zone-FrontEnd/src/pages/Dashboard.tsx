          ...
          <div className="bg-yellow-600/20 border border-yellow-600/50 rounded-xl p-4">
            ...
            <div className="space-y-2">
              {serviceRequests.map(req => (
                <div key={req.id} className="bg-gray-800 rounded-lg p-3 flex items-center justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-yellow-400">{req.deviceName}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        req.requestType === 'CallStaff' ? 'bg-orange-500/20 text-orange-400' : 'bg-purple-500/20 text-purple-400'
                      }`}>
          ...
          {rooms.map(room => {
            ...
            return (
              <div key={room.id} onClick={() => handleRoomClick(room.id)}
                ...
                {room.status === 'busy' && (
                  <button onClick={e => { e.stopPropagation(); handleRoomClick(room.id); }}
                    className="w-full mt-2 bg-white/10 hover:bg-white/20 backdrop-blur-md py-2 rounded-lg text-sm font-bold transition flex items-center justify-center gap-2">
                    <i className="fa-solid fa-bell"></i>
                    <span>عرض & استدعاء</span>
                  </button>
                )}
              </div>
            );
