...
export interface ServiceRequestItem {
  id: string; sessionId?: string; deviceId: string; deviceName: string;
  requestType: string; description: string; status: string; createdAt: string;
}
...
export function PlayZoneProvider({ children }: { children: ReactNode }) {
  ...
  const fetchServiceRequests = useCallback(async () => {
    try {
      const res = await api.get('/servicerequests/pending');
      setServiceRequests(res.data || []);
    } catch {}
  }, []);

  const acceptServiceRequest = useCallback(async (id: string) => {
    try {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [devRes, priceRes, todayRes, settingsRes, sessionsRes, serviceRes] = await Promise.all([
        ...
        api.get('/servicerequests/pending').catch(() => ({ data: [] })),
      ]);
  ...
  // Poll for new service requests every 10 seconds
  useEffect(() => {
    fetchServiceRequests();
    const iv = setInterval(fetchServiceRequests, 10000);
  ...
  const contextValue = useMemo(() => ({
    ...
    showAddDeviceModal, showEditDeviceModal, editingDevice,
    showAddMenuItemModal, showReceiptDetail, loading,
    editingMenuItem, notifications, serviceRequests,
  ]);
